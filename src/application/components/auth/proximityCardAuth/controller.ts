...
export default class ProximityCardAuthController extends AuthController
{
    ...
    handleCardInserted = (ev: CardInserted) => {
        ...
        this.reader.getCardInfo(ev.deviceId).then(card => {
            if (card && card.Type === CardType.Proximity) {
                this.card = card;
                super.emitOnBusy();
                this.reader.getCardAuthData(card.Reader)
                .then(data => {
                    const auth = new ProximityCardAuth(this.authService);
